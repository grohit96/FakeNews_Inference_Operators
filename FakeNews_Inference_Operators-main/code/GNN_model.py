...
def running_code(proc_id, n_gpus, args, devices, overall_graph):
    ...
    for curr_data_split_key in all_the_data_splits_we_want_to_run_now:
        ...
        dataloader_nc = dgl.dataloading.DataLoader(curr_g, {'source': train_nid}, sampler, batch_size=64, shuffle=True, drop_last=False, num_workers=args.num_workers)
        test_sampler = sampler
        dataloader_test = dgl.dataloading.DataLoader(curr_g, {'source': test_idx}, test_sampler, batch_size=64, shuffle=True, drop_last=False, num_workers=args.num_workers)
        dev_sampler = sampler
        dataloader_dev = dgl.dataloading.DataLoader(curr_g, {'source': dev_idx}, dev_sampler, batch_size=64, shuffle=True, drop_last=False, num_workers=args.num_workers)
        ...
        best_test_acc_at_dev_average.append(bes_acc_at_dev)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_where_data_is', type=str, default='/scratch/rganesh5/PaperGraph_release_connected/News-Media-Reliability')
    parser.add_argument('--dataset_corpus', type=str, default='/scratch/rganesh5/PaperGraph_release/News-Media-Reliability/data/acl2020/corpus.tsv')
